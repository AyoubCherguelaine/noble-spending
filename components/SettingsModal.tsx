    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(3,6,10,.62)', backdropFilter: 'blur(3px)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 420, maxWidth: '100%', background: '#12151a', border: '1px solid #1e242c', borderRadius: 4, boxShadow: '0 24px 60px rgba(0,0,0,.5)', animation: 'fadeUp .16s ease-out' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {[['USD', '$ USD'], ['DA', 'DA'], ['EUR', '€ EUR']].map(([id, label]) => (
                <button key={id} onClick={() => update('currency', id)} style={{
                  flex: 1, padding: '6px 4px', border: '1px solid ' + (form.currency === id ? '#2dd4bf' : '#1e242c'),
                  borderRadius: 4, cursor: 'pointer', font: '600 11px Space Grotesk, sans-serif',
                  background: form.currency === id ? '#2dd4bf' : 'transparent', color: form.currency === id ? '#04231e' : '#7d8794'
                }}>{label}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div>
                ...
                <input value={form.rate_eur} onChange={e => update('rate_eur', e.target.value)} style={{ width: '100%', height: 32, padding: '0 8px', background: '#0b0d10', border: '1px solid #1e242c', borderRadius: 4, color: '#e6edf3', font: '400 12px IBM Plex Mono, monospace', outline: 'none' }} />
              </div>
              <div>
                <div style={{ font: '400 10px IBM Plex Mono, monospace', color: '#7d8794', marginBottom: 4 }}>1 USD =</div>
                <input value={form.rate_da} onChange={e => update('rate_da', e.target.value)} style={{ width: '100%', height: 32, padding: '0 8px', background: '#0b0d10', border: '1px solid #1e242c', borderRadius: 4, color: '#e6edf3', font: '400 12px IBM Plex Mono, monospace', outline: 'none' }} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 8 }}>
              <div>
                <div style={{ font: '400 10px IBM Plex Mono, monospace', color: '#7d8794', marginBottom: 4 }}>1 EUR = (DA)</div>
                <input value={form.rate_eur_da} onChange={e => update('rate_eur_da', e.target.value)} style={{ width: '100%', height: 32, padding: '0 8px', background: '#0b0d10', border: '1px solid #1e242c', borderRadius: 4, color: '#e6edf3', font: '400 12px IBM Plex Mono, monospace', outline: 'none' }} />
              </div>
            </div>
        ...
        <div style={{ padding: '14px 18px', borderTop: '1px solid #1e242c', display: 'flex', justifyContent: 'flex-end', gap: 9 }}>
          ...
          <button onClick={submit} disabled={saving} style={{ height: 32, padding: '0 16px', background: '#2dd4bf', color: '#06251f', border: 'none', borderRadius: 4, font: '600 12px Space Grotesk, sans-serif', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>{t('save')}</button>
        </div>
      </div>
    </div>
  );
