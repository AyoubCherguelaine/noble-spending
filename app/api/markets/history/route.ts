...
export async function GET(request: Request) {
  ...
  try {
    const cached = await db.prepare("SELECT payload FROM external_cache WHERE provider = 'markets' AND cache_key = ? AND expires_at > datetime('now') LIMIT 1").get(cacheKey) as { payload: string } | undefined;
    if (cached) {
    ...
    await db.prepare('INSERT OR REPLACE INTO external_cache (provider, cache_key, payload, fetched_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('markets', cacheKey, payload, now.toISOString(), expires);
    return NextResponse.json({ points, stale: false });
  } catch (e) {
    const fallback = await db.prepare("SELECT payload FROM external_cache WHERE provider = 'markets' AND cache_key = ? LIMIT 1").get(cacheKey) as { payload: string } | undefined;
