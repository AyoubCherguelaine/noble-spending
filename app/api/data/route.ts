...
export async function GET(request: Request) {
  ...
  const debtsOwe = db.prepare("SELECT * FROM debts WHERE type = 'owe' AND status != 'paid'").all();
  const debtsOwed = db.prepare("SELECT * FROM debts WHERE type = 'owed' AND status != 'paid'").all();
