...
export async function GET() {
  await initSchema();
  const rows = await db.prepare('SELECT * FROM purchase_candidates ORDER BY created_at DESC').all();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const row = await db.prepare('SELECT * FROM purchase_candidates WHERE id = ?').get(result.lastInsertRowid);
