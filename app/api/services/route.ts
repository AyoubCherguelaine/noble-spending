...
export async function POST(request: Request) {
  try {
    ...
    const result = stmt.run(name.trim(), description || '', terms || '', amount, status || 'Active', next_invoice || '', month || new Date().getMonth() + 1, year || new Date().getFullYear(), currency || 'USD');
    ...
    db.prepare('INSERT INTO transactions (date, merchant, category, method, original_currency, original_amount, converted_amount, type, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(today, name.trim(), 'income', 'Service', entryCurrency, numAmount, converted, 'income', `Service: ${status || 'Active'}`);
...
export async function PUT(request: Request) {
  try {
    ...
    db.prepare('UPDATE services SET name = ?, description = ?, terms = ?, amount = ?, status = ?, next_invoice = ?, month = ?, year = ?, currency = ? WHERE id = ?').run(name, description, terms, amount, status, next_invoice, month, year, currency, id);
