...
export async function GET() {
  const rows = await db.prepare('SELECT * FROM settings').all() as { key: string; value: string }[];
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === 'changeCredentials') {
      ...
      await db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)").run('auth_username', newUsername);
      await db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)").run('auth_password_hash', newHash);
      return NextResponse.json({ success: true });
    }

    const stmt = db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)');
    for (const [k, v] of Object.entries(body)) await stmt.run(k, String(v));
    return NextResponse.json({ success: true });
  } catch (e) {
