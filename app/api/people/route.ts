...
export async function GET() {
  const rows = db.prepare('SELECT * FROM people ORDER BY name ASC').all();
...
export async function POST(request: Request) {
  try {
    ...
    const result = stmt.run(name.trim(), type || 'owe', note || '');
...
export async function PUT(request: Request) {
  try {
    ...
    db.prepare('UPDATE people SET name = ?, type = ?, note = ? WHERE id = ?').run(name, type, note, id);
